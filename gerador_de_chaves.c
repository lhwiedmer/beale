...
void completa_lista(struct lista_char* lista) {
    short int j = -2; unsigned char c = 33;
...
struct lista_char* gera_lista_de_chaves(char* n_arq) {
    ...
    if (fclose(ArqLivro) != 0) {
        printf("Erro ao fechar arquivo Livro Cifra na funcao gera_lista_de_chaves.\n");
        exit(2);
    }
...
void escreve_lista_no_arquivo(char* n_arq, struct lista_char* lista) {
    ...
    if (ArqChaves == NULL) {
        printf("Erro ao abrir arquivo de chaves na funcao escreve_lista_no_arquivo.\n");
        exit(1);
    }
    ...
    if (fclose(ArqChaves) != 0) {
        printf("Erro ao fechar arquivo de chaves na funcao escreve_lista_no_arquivo.\n");
        exit(2);
    }
}
...
struct lista_char* escreve_arquivo_na_lista(char* n_arq) {
    ...
    if (ArqChaves == NULL) {
        printf("Erro ao abrir arquivo de chaves na funcao escreve_arquivo_na_lista.\n");
        exit(1);
    }
    struct lista_char* lista = cria_lista_char();
    struct nodo_char* nodo;
    ...
    if (fclose(ArqChaves) != 0) {
        printf("Erro ao fechar arquivo de chaves na funcao escreve_arquivo_na_lista.\n");
